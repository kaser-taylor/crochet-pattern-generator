...
def define_sleeve_variables(swatch_data, measurements):
    ...
    sleeve_variables['calculated_sleeve_cap_rows'] = int(sleeve_variables['sleeve_cap_rows'] + sleeve_variables['sleeve_rows'])

    sleeve_variables['estimated_sleeve_stitches_at_top'] = int(measurements['upper arm circ'] / swatch_data['stitch_width'])

    sleeve_variables['stitches_at_wrist'] = int((measurements['wrist']) / swatch_data['stitch_width'])
    ...
    sleeve_variables['sleeve_rows_between_increase'] = int((sleeve_variables['sleeve_rows'] / (sleeve_variables['sleeve_total_increase'] / sleeve_variables['sleeve_stitch_increase'])) - 1)
...
def define_front_bodice_variables(swatch_data, measurements):
    ...
    bodice_variables['hip_to_waist_rows'] 
